...
public class RegistrosMotos extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_registros_motos);
        ...
        opc=res.getStringArray(R.array.opciones_modelo);
        ...
        opc2=res.getStringArray(R.array.opciones_cilindraje);
        ...
        cajaMetros.addTextChangedListener(new TextWatcherPersonalizado(icajaMetros,res.getString(R.string.error_reserva)) {
            @Override
    ...
    public void onBackPressed() {
        finish();
        Intent i=new Intent(RegistrosMotos.this,Principal.class);
        startActivity(i);
    }
    ...
    public boolean validarTodo(){
        if (cajaMetros.getText().toString().isEmpty()){
            cajaMetros.setError(res.getString(R.string.error_reserva));
    ...
    public boolean validarExistente(){
        ArrayList<Moto> a= DatosMotos.traerApartamentos(getApplicationContext());
        if (a.size()!=0){
    ...
    public boolean validarPiso(){
        ...
        if (cont>=3){
            Toast.makeText(getApplicationContext(),res.getString(R.string.error_cilindraje),
                    Toast.LENGTH_SHORT).show();
            return false;
        }
        return true;
        ...
        if (validarTodo()){
            if (validarExistente()){
                if (validarPiso()){
                    ...
                    if (rbBalcon1.isChecked())balcon=res.getString(R.string.tiene_freno);
                    else balcon=res.getString(R.string.no_tiene_freno);
                    if (rbSombra1.isChecked())sombra=res.getString(R.string.tiene_luces);
                    else sombra=res.getString(R.string.no_tiene_luces);

                    Moto a=new Moto(foto,nom,piso,metros,precio,balcon,sombra);
